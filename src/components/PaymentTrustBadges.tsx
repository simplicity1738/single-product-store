...
const badgeClassName =
  "h-7 w-auto shrink-0 sm:h-8 opacity-80 transition-opacity duration-300 hover:opacity-100";

const monochromeFilterClassName =
  "grayscale brightness-0 opacity-70 hover:opacity-90";

function badgeWrapClassName(monochrome: boolean): string {
  return `${badgeClassName} ${monochrome ? monochromeFilterClassName : ""}`;
}

function StripeLogo({ monochrome }: { monochrome?: boolean }) {
  return (
    <svg
      viewBox="0 0 64 24"
      className={badgeWrapClassName(Boolean(monochrome))}
      role="img"
      aria-label="Stripe"
    >
      <text
...
function VisaLogo({ monochrome }: { monochrome?: boolean }) {
  return (
    <svg
      viewBox="0 0 24 24"
      className={badgeWrapClassName(Boolean(monochrome))}
...
function MastercardLogo({ monochrome }: { monochrome?: boolean }) {
  return (
    <svg
      viewBox="0 0 48 30"
      className={`${badgeWrapClassName(Boolean(monochrome))} max-w-[2.75rem] sm:max-w-none`}
      role="img"
...
function ApplePayLogo({ monochrome }: { monochrome?: boolean }) {
  return (
    <svg
      viewBox="0 0 120 40"
      className={`${badgeWrapClassName(Boolean(monochrome))} max-w-[3.5rem] sm:max-w-none`}
      role="img"
...
function KlarnaBadge({ monochrome }: { monochrome?: boolean }) {
  return (
    <span
      className={`inline-flex items-center justify-center rounded-full bg-[#FFB3C7] px-4 py-1.5 font-sans text-[10px] font-black uppercase tracking-widest text-[#0A0A0A] opacity-80 transition-opacity duration-300 hover:opacity-100 ${
        monochrome ? "grayscale brightness-75" : ""
      }`}
      role="img"
      aria-label="Klarna"
    >
...
function BitcoinLogo({ monochrome }: { monochrome?: boolean }) {
  return (
    <svg
      viewBox="0 0 24 24"
      className={`${badgeWrapClassName(Boolean(monochrome))} max-w-[2rem] sm:max-w-none`}
      role="img"
      aria-label="Bitcoin"
    >
      <circle cx="12" cy="12" r="11" fill="#F7931A" fillOpacity="0.12" />
      <path
        fill="#F7931A"
...
type PaymentBadgeItem = {
  key: string;
  render: (monochrome: boolean) => React.ReactNode;
};

const DEFAULT_BADGES: PaymentBadgeItem[] = [
  { key: "stripe", render: (monochrome) => <StripeLogo monochrome={monochrome} /> },
  { key: "visa", render: (monochrome) => <VisaLogo monochrome={monochrome} /> },
  {
    key: "mastercard",
    render: (monochrome) => <MastercardLogo monochrome={monochrome} />,
  },
  {
    key: "apple-pay",
    render: (monochrome) => <ApplePayLogo monochrome={monochrome} />,
  },
  { key: "klarna", render: (monochrome) => <KlarnaBadge monochrome={monochrome} /> },
  { key: "bitcoin", render: (monochrome) => <BitcoinLogo monochrome={monochrome} /> },
];
  ...
  return (
    <div className={`text-center ${className}`}>
      ...
      <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-3 sm:gap-x-7">
        {badges.map((badge) => (
          <span key={badge.key} className="inline-flex items-center">
            {badge.render(monochrome)}
          </span>
        ))}
      </div>
    </div>
