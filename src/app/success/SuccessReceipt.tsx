...
export default function SuccessReceipt() {
  ...
  const isStripePaid =
    searchParams.get("paid") === "stripe" || order?.paymentMethod === "stripe";

  const title = isStripePaid ? t.success.stripeTitle : t.success.bitcoinTitle;
  const subtitle = isStripePaid
    ? t.success.stripeSubtitle
    : t.success.bitcoinSubtitle;
  ...
  const statusBadge = useMemo(() => {
    if (isStripePaid) {
      return {
        label: t.success.paidStatus,
        className:
          "inline-flex items-center gap-2 rounded-full border border-emerald-300 bg-emerald-50 px-4 py-2 text-sm font-semibold text-emerald-900 shadow-sm",
        iconClassName: "bg-emerald-100 text-emerald-600",
      };
    }

    return {
      label: t.success.pendingStatus,
      className:
        "inline-flex items-center gap-2 rounded-full border border-amber-300 bg-amber-50 px-4 py-2 text-sm font-semibold text-amber-900 shadow-sm",
      iconClassName: "bg-amber-100 text-amber-600",
    };
  }, [isStripePaid, t.success.paidStatus, t.success.pendingStatus]);
    ...
    <div className="min-h-full bg-gradient-to-b from-rose-50 via-white to-rose-50/30">
      ...
      <main className="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16">
        <div className="text-center">
          <div
            className={`mx-auto flex h-16 w-16 items-center justify-center rounded-full ${statusBadge.iconClassName}`}
          >
            {isStripePaid ? (
              <svg
                className="h-8 w-8"
          ...
          <p className="mt-3 text-lg leading-relaxed text-zinc-600">{subtitle}</p>
        </div>
        ...
        {order && (
          <div
            className={`mt-8 rounded-2xl border px-6 py-5 text-center sm:text-left ${
              isStripePaid
          ...
          >
            <p
              className={`text-sm font-medium ${
                isStripePaid ? "text-emerald-950" : "text-amber-950"
              }`}
            >
